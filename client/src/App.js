...
function App() {
  ...
  useEffect(() => {
    let table_list = []
    let graph_input = []
    fetch('http://localhost:3001/get_all_tables')
        .then(response => response.json())
        .then(output => {
          console.log(output)
          for (let table in output["message"]){
            let temp_list = []
            for(var i in output["message"][table]){
              temp_list.push([output["message"][table][i]["time"], output["message"][table][i]["value"]]);
            }
            graph_input.push([table, temp_list])
            table_list.push(table)
            console.log(graph_input)
          }
          console.log(graph_input)
          setData(graph_input)
          setCurrentTables(table_list)
          setColours(generateRandomColors(1,generateRandomColors(table_list.length - 1)))
          })
  }, []);
  ...
  return (
    <div className="App">
      <Header setShowApiOverlay={() => setShowApiOverlay(!showApiOverlay)} />
      <APIOverlay showApiOverlay={showApiOverlay} setShowApiOverlay={() => setShowApiOverlay(!showApiOverlay)} currentTables={currentTables} />
      {!data ? "Loading.. " : <Graph data={data} colours={colours}/>}
    </div>
  );
}
